...
namespace ProjectAI.Tests
{
    public class GameLogicTestRunner
    {
        ...
        private static bool RunLateJoinerOrderTest()
        {
            ...
            // 의존성 주입 (Awake 시뮬레이션)
            FieldInfo statHealthField = typeof(NetStatComponent).GetField("healthComponent", BindingFlags.NonPublic | BindingFlags.Instance);
            if (statHealthField != null)
            {
                statHealthField.SetValue(statComp, healthComp);
            }
            
            FieldInfo statEventsField = typeof(NetStatComponent).GetField("entityEvents", BindingFlags.NonPublic | BindingFlags.Instance);
            if (statEventsField != null)
            {
                statEventsField.SetValue(statComp, entityEvents);
            }
        ...
        private static bool RunBasicAttackHitTest()
        {
            ...
            NetCharacter casterCharacter = casterGo.AddComponent<NetCharacter>();
            NetStatComponent casterStat = casterGo.AddComponent<NetStatComponent>();
            NetSkillComponent casterSkill = casterGo.AddComponent<NetSkillComponent>();
            
            GameObject targetGo = new GameObject("Target");
            NetHealthComponent targetHealth = targetGo.AddComponent<NetHealthComponent>();
            NetStatComponent targetStat = targetGo.AddComponent<NetStatComponent>();
            NetEntity targetEntity = targetGo.AddComponent<NetEntity>();
            
            // Entity 연결
            targetHealth.SetOwner(targetEntity);
            PropertyInfo targetStatOwnerField = typeof(NetStatComponent).GetProperty("OwnerEntity");
            ...
            {
               FieldInfo field = typeof(NetStatComponent).GetField("<OwnerEntity>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);
               if(field != null)
               {
                   field.SetValue(targetStat, targetEntity);
               }
            }
            
            PropertyInfo targetEntityStatProp = typeof(NetEntity).GetProperty("StatComponent");
            if (targetEntityStatProp != null && targetEntityStatProp.CanWrite)
            {
                targetEntityStatProp.SetValue(targetEntity, targetStat);
            }
            else
            {
                FieldInfo field = typeof(NetEntity).GetField("<StatComponent>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);
                if(field != null)
                {
                    field.SetValue(targetEntity, targetStat);
                }
            }
            ...
            // Hitbox 구성
            BoxCollider2D casterHitbox = casterGo.AddComponent<BoxCollider2D>();
            casterHitbox.isTrigger = true;
            casterHitbox.size = new Vector2(5, 1); // X는 길게, Y(깊이)는 1로 설정하여 extents.y = 0.5 유지
            
            BoxCollider2D targetCollider = targetGo.AddComponent<BoxCollider2D>();
            targetCollider.isTrigger = true;
            targetCollider.size = new Vector2(1, 1);
            
            // Reflection으로 MeleeHitbox 세팅
            PropertyInfo meleeHitboxProp = typeof(NetSkillComponent).GetProperty("MeleeHitbox");
            ...
            {
                FieldInfo hitboxField = typeof(NetSkillComponent).GetField("<MeleeHitbox>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);
                if (hitboxField != null)
                {
                    hitboxField.SetValue(casterSkill, casterHitbox);
                }
            }
            
            PropertyInfo statProp = typeof(NetCharacter).GetProperty("StatComponent");
            if (statProp != null && statProp.CanWrite)
            {
                statProp.SetValue(casterCharacter, casterStat);
            }
            else
            {
                FieldInfo statField = typeof(NetCharacter).GetField("<StatComponent>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);
                if (statField != null)
                {
                    statField.SetValue(casterCharacter, casterStat);
                }
            }

            PropertyInfo skillProp = typeof(NetCharacter).GetProperty("SkillComponent");
            if (skillProp != null && skillProp.CanWrite)
            {
                skillProp.SetValue(casterCharacter, casterSkill);
            }
            else
            {
                FieldInfo skillField = typeof(NetCharacter).GetField("<SkillComponent>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);
                if (skillField != null)
                {
                    skillField.SetValue(casterCharacter, casterSkill);
                }
            }
            ...
            BasicAttackLogic logic = new BasicAttackLogic();
            BaseSkillConfig config = ScriptableObject.CreateInstance<BaseSkillConfig>();
